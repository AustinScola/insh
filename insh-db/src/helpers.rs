...
/// Return a `LIKE` pattern which matches the strings that contain some text anywhere.
///
/// The characters which are special to `LIKE` are escaped with a backslash, which is the escape
/// character that `LIKE` uses when it is not given another one.
pub fn like_contains_pattern(text: &str) -> String {
    let mut pattern: String = String::with_capacity(text.len() + 2);
...
mod tests {
    ...
    #[test_case("foo", "%foo%"; "text with nothing to escape")]
    ...
    fn test_like_contains_pattern(text: &str, expected: &str) {
        assert_eq!(like_contains_pattern(text), expected);
    }
