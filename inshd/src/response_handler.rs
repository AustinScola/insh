...
pub struct ResponseHandler {
    ...
    contexted_responses_rx: Receiver<ContextedResponse>,

    /// Receives updates about requests made by clients.
    client_requests_rx: Receiver<ClientRequest>,

    /// Receives new clients.
    new_clients_rx: Receiver<Client>,
    /// Receives updates about clients disconnecting.
    ...
    /// How many responses have been handled for each client.
    #[builder(setter(skip), default)]
    ...
    pub fn run(&mut self) {
        ...
        loop {
            select! {
                ...
                recv(self.contexted_responses_rx) -> contexted_response => {
                    let contexted_response: ContextedResponse = match contexted_response {
                        Ok(contexted_response) => contexted_response,
                        Err(error) => {
                    ...
                    // NOTE: Responses which carry log records are not logged. If they were, then
                    // sending one would emit log records, which would be sent as more responses,
                    // which would emit more log records, and so on forever.
                    let log: bool = contexted_response.log();
                    let response: &Response = contexted_response.response();
                    ...
                    // TODO: Re-use the response buffer.
                    if log {
